...
6. CI policy failures (`unpinned action` / `pipe-to-shell` / `permissions: write-all` / `pull_request_target`): inspect `.github/workflows/ci-change-audit.yml` summary + artifact.
7. Provider connectivity drift/incidents: inspect `.github/workflows/ci-provider-connectivity.yml` summary + artifact.
8. Reproducibility drift signals: inspect `.github/workflows/ci-reproducible-build.yml` artifacts.
9. Provenance/signing failures: inspect `.github/workflows/ci-supply-chain-provenance.yml` logs and bundle artifacts.
10. Rollback planning/execution issues: inspect `.github/workflows/ci-rollback.yml` summary + `ci-rollback-plan` artifact.
11. Workflow syntax/lint failures: inspect `.github/workflows/workflow-sanity.yml`.
12. PR intake failures: inspect `.github/workflows/pr-intake-checks.yml` sticky comment and run logs.
13. Label policy parity failures: inspect `.github/workflows/pr-label-policy-check.yml`.
14. Docs failures in CI: inspect `docs-quality` job logs in `.github/workflows/ci-run.yml`.
15. Strict delta lint failures in CI: inspect `lint-strict-delta` job logs and compare with `BASE_SHA` diff scope.
16. Suspected flaky tests: inspect `Test Flake Retry Probe` summary and `test-flake-probe` artifact in `.github/workflows/ci-run.yml`.
